	...
	FILE *ptarName;
		...
		if(ptarName!=NULL){
			...
			if(pheaders!=NULL){
				...
				for(int i = 0; i < numFiles;i++){
					buffer = malloc(sizeof(char)* pheaders[i].size);
					fread(buffer,sizeof(char),pheaders[i].size,ptarName);
					resultado = memcrc(buffer,pheaders[i].size);
					printf("FICHERO(%d): %u",i,resultado);
					printf("\n");
				}
				fclose(ptarName);
			}
